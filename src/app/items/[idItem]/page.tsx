        ...
        <div className="max-w-md mx-auto">
          <div className="bg-white shadow-lg rounded-lg overflow-hidden my-4">
            <div className="w-full h-64">
              <img src={items.img} alt={items.Name} className="w-full h-full object-contain" />
            </div>
            <div className="p-4">
              <h2 className="text-2xl font-bold text-gray-800">{items.Name}</h2>
              <p className="mt-2 text-gray-600 line-clamp-3">{items.description}</p>
              <a href="#fullDescription" className="text-blue-500 hover:text-blue-700 cursor-pointer">Lire la suite</a>
              <div className="flex justify-between items-center mt-3">
                <span className="text-3xl font-bold text-green-600">{items.price.toFixed(2)} €</span>
                <button className="px-4 py-2 bg-fontDesktop text-white text-sm font-semibold rounded uppercase hover:bg-green-600">Ajouter au panier</button>
              </div>
            </div>
          </div>
          <div className="bg-white shadow-lg rounded-lg px-4 py-2 mb-24">
            <details className="group">
              <summary id="fullDescription" className="text-lg font-bold text-gray-800 cursor-pointer">Description <span className="float-right transition-transform duration-300 group-open:rotate-180">➕</span></summary>
